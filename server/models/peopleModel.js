...
export class PeopleModel {
    ...
    getPersonById(id) {
        ...
        return new Promise((resolve, reject) => {
            this.db.query("SELECT get_person_details_full($1) AS data", [id])
            .then(responseJSON => {
    ...
    getPeopleList(page, limit, filters) {
        logger.debug("Enter peopleModel.GetPeopleList");

        return new Promise((resolve, reject) => {
            this.db.query("SELECT * FROM a_person")
            .then(response => {
                if (response != null) {
                    logger.debug("Model received valid response from DB");
                    //console.log("non null response. Return promise");
    ...
    updatePersonById(id, patchJSON) {
        ...
        return new Promise((resolve, reject) => {
            this.db.query("SELECT update_person_details_partial($1, $2)", [id, inputData])
            .then(responseJSON => {
                if (responseJSON != null) {
                    logger.debug("Model received valid response from DB");
                    //console.log("non null response. Return promise");
    ...
    getTreeFocusData(id) {
        ...
        return new Promise((resolve, reject) => {
            this.db.query("SELECT get_person_tree_snapshot_window($1) AS data", [id])
            .then(responseJSON => {
                if (responseJSON != null) {
                    logger.debug("Received response from DB");
